...
NSString *const ELAsyncSocketException = @"AsyncSocketException";
NSString *const ELAsyncSocketErrorDomain = @"AsyncSocketErrorDomain";


enum ELAsyncSocketFlags
{
...
@interface ELAsyncSocket (Private)
...
**/
@interface ELAsyncReadPacket : NSObject
{
  @public
...
@implementation ELAsyncReadPacket
...
**/
@interface ELAsyncWritePacket : NSObject
{
  @public
	NSData *buffer;
	NSUInteger bytesDone;
...
@implementation ELAsyncWritePacket
...
**/
@interface ELAsyncSpecialPacket : NSObject
{
  @public
...
@implementation ELAsyncSpecialPacket
...
- (void)checkForThreadSafety
{
	if (theRunLoop && (theRunLoop != CFRunLoopGetCurrent()))
	{
		...
		[NSException raise:ELAsyncSocketException
...
- (float)progressOfReadReturningTag:(long *)tag bytesDone:(NSUInteger *)done total:(NSUInteger *)total
{
	...
	if (!theCurrentRead || ![theCurrentRead isKindOfClass:[ELAsyncReadPacket class]])
	{
		if (tag != NULL)   *tag = 0;
		if (done != NULL)  *done = 0;
...
- (float)progressOfWriteReturningTag:(long *)tag bytesDone:(NSUInteger *)done total:(NSUInteger *)total
{
	...
	if (!theCurrentWrite || ![theCurrentWrite isKindOfClass:[ELAsyncWritePacket class]])
	{
		if (tag != NULL)   *tag = 0;
		if (done != NULL)  *done = 0;
		if (total != NULL) *total = 0;
...
- (BOOL)createSocketForAddress:(NSData *)remoteAddr error:(NSError **)errPtr
{
	...
	else
	{
		if (errPtr)
		{
			...
			*errPtr = [NSError errorWithDomain:ELAsyncSocketErrorDomain code:AsyncSocketCFSocketError userInfo:info];
		}
		return NO;
	}
...
- (void)doAcceptFromSocket:(CFSocketRef)parentSocket withNewNativeSocket:(CFSocketNativeHandle)newNativeSocket
{
	if(newNativeSocket)
	{
		...
		ELAsyncSocket *newSocket = [[[self class] alloc] initWithDelegate:theDelegate];
...
- (void)recoverUnreadData
{
	if(theCurrentRead != nil)
	{
		...
		if(([theCurrentRead isKindOfClass:[ELAsyncReadPacket class]]) && (theCurrentRead->bytesDone > 0))
		{
...
- (NSError *)getSocketError
{
	...
	return [NSError errorWithDomain:ELAsyncSocketErrorDomain code:AsyncSocketCFSocketError userInfo:info];
}
...
- (NSError *)getAbortError
{
	...
	return [NSError errorWithDomain:ELAsyncSocketErrorDomain code:AsyncSocketCanceledError userInfo:info];
}
...
- (NSError *)getConnectTimeoutError
{
	...
	return [NSError errorWithDomain:ELAsyncSocketErrorDomain code:AsyncSocketConnectTimeoutError userInfo:info];
}
...
- (NSError *)getReadMaxedOutError
{
	...
	return [NSError errorWithDomain:ELAsyncSocketErrorDomain code:AsyncSocketReadMaxedOutError userInfo:info];
}
...
- (NSError *)getReadTimeoutError
{
	...
	return [NSError errorWithDomain:ELAsyncSocketErrorDomain code:AsyncSocketReadTimeoutError userInfo:info];
}
...
- (NSError *)getWriteTimeoutError
{
	...
	return [NSError errorWithDomain:ELAsyncSocketErrorDomain code:AsyncSocketWriteTimeoutError userInfo:info];
}
...
- (NSString *)description
{
	...
	if (theCurrentRead == nil || [theCurrentRead isKindOfClass:[ELAsyncSpecialPacket class]])
	...
	if (theCurrentWrite == nil || [theCurrentWrite isKindOfClass:[ELAsyncSpecialPacket class]])
                        ...
                        tag:(long)tag
{
	...
	ELAsyncReadPacket *packet = [[ELAsyncReadPacket alloc] initWithData:buffer
	                                                    startOffset:offset
	                                                      maxLength:length
	                                                        timeout:timeout
	                                                     readLength:0
...
- (void)maybeDequeueRead
{
	...
	if((theCurrentRead == nil) && (theReadStream != NULL))
	{
		if([theReadQueue count] > 0)
		{
			...
			if([theCurrentRead isKindOfClass:[ELAsyncSpecialPacket class]])
			{
...
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	ELAsyncWritePacket *packet = [[ELAsyncWritePacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (void)maybeDequeueWrite
{
	...
	if((theCurrentWrite == nil) && (theWriteStream != NULL))
	{
		if([theWriteQueue count] > 0)
		{
			...
			if([theCurrentWrite isKindOfClass:[ELAsyncSpecialPacket class]])
			{
				// Attempt to start TLS
...
- (void)startTLS:(NSDictionary *)tlsSettings
{
	...
	ELAsyncSpecialPacket *packet = [[ELAsyncSpecialPacket alloc] initWithTLSSettings:tlsSettings];
...
- (void)maybeStartTLS
{
	...
	{
		ELAsyncSpecialPacket *tlsPacket = (ELAsyncSpecialPacket *)theCurrentRead;
...
static void MyCFSocketCallback (CFSocketRef sref, CFSocketCallBackType type, CFDataRef inAddress, const void *pData, void *pInfo)
{
	@autoreleasepool {
		...
		ELAsyncSocket *theSocket = (__bridge ELAsyncSocket *)pInfo;
